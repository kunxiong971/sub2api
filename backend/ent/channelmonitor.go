...
type ChannelMonitor struct {
	...
	// true = judge by non-empty response body (image generation); false = text-path based (default)
	ImageMode bool `json:"image_mode,omitempty"`
...
func (*ChannelMonitor) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case channelmonitor.FieldExtraModels, channelmonitor.FieldExtraHeaders, channelmonitor.FieldBodyOverride:
			values[i] = new([]byte)
		case channelmonitor.FieldEnabled, channelmonitor.FieldImageMode:
...
func (_m *ChannelMonitor) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case channelmonitor.FieldImageMode:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field image_mode", values[i])
			} else if value.Valid {
				_m.ImageMode = value.Bool
			}
		default:
...
func (_m *ChannelMonitor) String() string {
	...
	builder.WriteString(", ")
	builder.WriteString("image_mode=")
	builder.WriteString(fmt.Sprintf("%v", _m.ImageMode))
	builder.WriteByte(')')
