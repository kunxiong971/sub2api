...
| 项 | 内容 |
|---|---|
| **90 秒必刷新的真正根因** | 后端每次返回配置都重新签发登录票据（JWT 带 iat）→ 旧版壳页面把票据拼进 iframe 地址 → 每 90 秒轮询拿到新票据 → 地址变化 → **iframe 每 90 秒被重载**。fix4 已固定地址（不复算）；用户仍在旧面板页跑旧代码故持续复现（iframe 重载不刷新外层页面） |
| fix5 内容 | 面板新版本探测：每 90 秒轻量比对首页 ETag，发现新部署时给一次性可关闭提示「系统已更新，刷新页面以使用最新版本」（用户手动刷新，避免自动刷新打断对话）；镜像 `sub2api:fix5-20260915`（当前线上） |
| 服务端 nginx 缓存修正 | 画布 vhost 与生图台 conf 的 index.html 增加 no-cache、js/css 长缓存规则修正（此前双反斜杠导致规则失效，出现过旧页面 404 打不开） |
| 本地/服务器清理 | 删除全部镜像 tar、构建日志、临时脚本、误创建文件；服务器 /tmp 中间产物清理（/root 备份全部保留） |
| **git 存档（2026-09-15）** | sub2api `9571b7fac`（工作台加固批次全部改动）；gpt_image_playground（5 文件）；infinite-canvas（6 文件）；lobe-chat 拆两个提交：`748c8faaa4`（长上下文提醒+文字清晰化）、`5247c854f1`（fork 本地裁剪快照，对应 v8 镜像工作区） |
| 交接 | 下会话交接提示词见 `docs/HANDOFF-2026-09-15.md` |
